...
export const register = async (req, res) => {
  try {
    ...
    // Las credenciales son correctas, se puede generar el token de acceso y el token de actualización
    const accessToken = generateAccessToken({ id: saveUser.id });
    const refreshToken = generateRefreshToken({ id: saveUser.id });

    res.json({ accessToken, refreshToken, user: saveUser });
  } catch (error) {
    console.log(error);
    res.status(400).json({ message: "DB Error" });
  }
